...
class TestLock (object):
    ...
    def test_lock_config (self):
        ...
        class Minimo (ChimeraObject):
            ...
            @lock
            def doWriteLocked (self):
                """
                This method is synchdonized (@lock) so, it hold a lock
                on object monitor and a exclusive write on object
                configurations.
                """
                for i in range(1000):
                    self["config"] = i
                    print "[ locked write ] - config=%d" % i
                    sys.stdout.flush()
                    
                    if not (i % 100):
                        print "[ locked interleaved read ] - config=%d" % self["config"]
                        sys.stdout.flush()

            def doWriteUnlocked (self):
                """
                This method doesn't have exclusive rights on nothing,
                so it will fight with doReadUnlocked to access config
                values.
                """
                for i in range(1000):
                    self["config"] = i
                    print "[ unlocked write ] - config=%d" % i
                    sys.stdout.flush()
                    
                    if not (i % 100):
                        print "[ unlocked interleaved read ] - config=%d" % self["config"]
                        sys.stdout.flush()

            def doReadUnlocked (self):

                for i in range(1000):
                    t0 = time.time()
                    value = self["config"]
                    t = time.time()-t0               
                    print "[  unlocked read ] - config=%s took %.6f" % (value, t)
                    sys.stdout.flush()

                    if not (i%100):
                        self["config"] = i
                        print "[ unlocked interleaved write ] - config=%d" % self["config"]
                        sys.stdout.flush()                        
                        

        m = Minimo()

        t1 = threading.Thread(target=lambda: m.doWriteLocked())
        t2 = threading.Thread(target=lambda: m.doWriteUnlocked())
        t3 = threading.Thread(target=lambda: m.doReadUnlocked())
